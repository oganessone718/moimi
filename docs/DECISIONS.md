...
| D9 | MVP 범위 | **타임존(F8) 제거→v1.1**. 출발지(F6) **옵셔널·부가**. 중간지점은 MVP 유지(Director). 시간조율(F7) 유지 | Braintrust P0 + Director 조정 |
...
- 다음: 외부 셋업(SETUP.md) → 배포 후 ①②③ PoC → **S1 데이터 모델**(PLAN §5, `src/lib/db/schema.ts`에 테이블 정의 + 마이그레이션).
